...
Tuple compute_row(double r, double rc, double alpha, double eps){
    ...
    row.pe = alpha * eps * (ri2 - 1.) * (rcri2 - 1.) * (rcri2 - 1.); // Potential energy
    row.f = 2.*alpha * eps *ri2*ri*(rcri2 - 1.)*(3.*rcri2 - 2.*rc2 -1.); // force
...
int main(int argc, char* argv[]){
    ...
    if(argc!=6){
        printf("Require 5 args: L (box length), resolution, rc (cutoff), eps (well depth), filename. Exiting.\n");
        return 1;
    }
    ...
    // get filename from argv[5] directly
    ...
    while(r<L/2.){
        ...
        if (r>rc){
            ...
            // calculate potential
            Tuple row = compute_row(r, rc, alpha, eps);
            fprintf(fp, "%i %lf %lf %lf\n", i, r, row.pe, row.f);
        }
    }
    // Write actual N value
